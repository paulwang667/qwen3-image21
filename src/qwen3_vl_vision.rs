//! Qwen3-VL vision tower, vendored from candle-transformers 0.11
//! (`models/qwen3_vl/{vision,conv3d_temporal_2,config}.rs`, MIT/Apache-2.0),
//! whose `vision` module is private there. Returns the merged patch embeddings
//! plus the DeepStack features that the text model injects into its first layers.
#![allow(clippy::cast_possible_truncation, clippy::cast_precision_loss)]

use candle_core::{DType, Device, IndexOp, Module, Result, Tensor, D};
use candle_nn::{
    embedding, layer_norm, linear, Activation, Conv2d, Conv2dConfig, Embedding, LayerNorm, LayerNormConfig, Linear,
    VarBuilder,
};
...
impl PatchMerger {
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        // Upstream's merger uses exact `nn.GELU()` (candle's original used the tanh approximation).
        let xs = xs.gelu_erf()?;
        self.fc2.forward(&xs)
    }
}
