...
fn load_transformer(
    ...
    // safetensors path (non-quantized)
    eprintln!("  Loading transformer from safetensors (dtype={:?})...", dtype);
    let vb = unsafe { VarBuilder::from_mmaped_safetensors(&[model_path.to_string()], dtype, device)? };
    let cfg = TransformerConfig::default();
