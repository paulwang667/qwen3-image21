...
- **Only the non-quantized path (`transformer.rs`) has been verified end-to-end.** `quantized_transformer.rs` got the MLP and attention-scale fixes but still lacks the per-token `causal_condition` modulation, so expect worse output from GGUF models.
- **No CFG / negative prompt**: denoising is conditional-only (upstream supports true CFG).
