...
| `--kv-cache-cpu` | off | Keep the prefix KV cache in host memory and copy each layer's entries to the GPU only while that layer runs: saves ~2.2 GB of GPU memory per 1024² reference, bit-identical output, ~7–10 s slower per image. |
...
| Q4_K_M + `--flash-attn` + `--kv-cache-cpu` | **6.9** | 6 s | 28 s | 39 s |
| Q4_K_M + `--flash-attn` + `--kv-cache-cpu`, two 1024² references, 768×1024 output | **8.0** | 7 s | 34 s | 46 s |

- **For a 12 GB GPU**: Q4_K_M with `--flash-attn`. That covers text-to-image and single-reference edits at 1024²; for two 1024² references add `--kv-cache-cpu` (8.0 GiB, ~10 s slower) or `--output-resolution 768` (9.3 GiB), since without either they peak at 11.9 GiB, above a 12 GB card's usable ~11.2 GiB. With `--kv-cache-cpu` a single-reference edit peaks at 6.9 GiB, which may fit an 8 GB card (not tested on one). Without flash attention (older GPUs, or builds without `--features flash-attn`) a single-reference edit peaks at 9.2 GiB. The text encoder needs neither BF16 nor the CPU to fit; `--text-encoder-cpu` is only a fallback.
